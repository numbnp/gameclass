...
{$IFDEF EH_LIB_16} Themes, {$ENDIF}
  ...
  DataDriverEh, EhLibLCL, FrameMainGrid,
//  PrnDbgeh,
...
var
  FilePath: String;
begin
  if FileExistsUTF8(ExtractFilePath(ParamStr(0))+'\DBTest.mdb') { *Converted from FileExists* } then
    FilePath := ExtractFilePath(ParamStr(0))+'\DBTest.mdb'
  else if FileExistsUTF8(ExtractFilePath(ParamStr(0))+'..\Data\DBTest.mdb') { *Converted from FileExists* } then
...
var
  FilePath: String;
begin
  if FileExistsUTF8(ExtractFilePath(ParamStr(0))+'\BioLife.mdb') { *Converted from FileExists* } then
    FilePath := ExtractFilePath(ParamStr(0))+'\BioLife.mdb'
  else if FileExistsUTF8(ExtractFilePath(ParamStr(0))+'..\Data\BioLife.mdb') { *Converted from FileExists* } then
...
begin
  ...
  ODBCConnection2.Params[0] := 'DBQ='+FilePath;
end;
...
begin
{  SQLQuery1.Close;
  SQLQuery1.SQL.Text := Command.CommandText.Text;
  SQLQuery1.Open;
