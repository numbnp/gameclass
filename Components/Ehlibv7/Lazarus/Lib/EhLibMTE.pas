...
var
  ...
  DescDirections: TBooleanDynArray;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SetLength(DescDirections, DBGridEh.SortMarkedColumns.Count);
        for i := 0 to DBGridEh.SortMarkedColumns.Count - 1 do
          DescDirections[i] := DBGridEh.Center.GetSortOrderForSortMarker(DBGridEh,
            DBGridEh.SortMarkedColumns[i], DBGridEh.SortMarkedColumns[i].Title.SortMarker) = soDescEh;
        MTE.SortWithExternalProc(OrderValueProc, DescDirections);
      end else
...
begin
  ...
  begin
    ...
    Result := inherited LocateText(AGrid, FieldName, Text, AOptions,
      Direction, Matching, TreeFindRange, TimeOut);
end;
