...
begin
  ...
  Rows := (NewHeight - BorderSize - TitleRowHeight - FSpecRowHeight) div TextHeight;
  if Rows < 1 then Rows := 1;
  FRowCount := Rows;
  AddLine := 0;
  if dgRowLines in inherited Options then Inc(AddLine, GridLineWidth);
  NewWidth := NewWidth;
  NewHeight := Rows * TextHeight + BorderSize + TitleRowHeight + FSpecRowHeight + AddLine;
  Result := True;
end;
{$ENDIF}
...
(*
procedure TDBLookupGridEh.UpdateScrollBars;
var
  Pos, Max: Integer;
  Page: Cardinal;
  ScrollInfo: TScrollInfo;
...
//  if UseRightToLeftAlignment then ChangeBiDiModeAlignment(AAlignment);
  ...
  begin
    ...
    Windows.DrawFocusRect(Canvas.Handle, BoxRect(FixedColCount, ARow, ColCount-1, ARow));
  end;
end;
...
//            GetScrollInfo(Self.Handle, SB_VERT, SI);
            ...
            if Pos < OldRecNo then
              ...
              MoveBy(Pos - OldRecNo + ListLink.RecordCount - 1)
            else
...
//const
//  GripPositions: array[TSizeGripChangePosition] of TSizeGripPosition =
