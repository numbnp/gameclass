...
uses
  ...
  ADODataDriverEh, ADODB, DBAxisGridsEh, StdCtrls, ExtCtrls;
...
type
  ...
  public
    { Public declarations }
    procedure AppendSQLInfo(Command: TCustomSQLCommandEh); overload;
    procedure AppendSQLInfo(Command: ADODB._Command); overload;
  end;
...
procedure TfSQLMon.AppendSQLInfo(Command: _Command);
var
  ap: Parameters;
  i: Integer;
  s, ps: String;
begin
  MemTableEh1.Append;
  MemTableEh1['SQLText'] := Command.CommandText;
  ap := Command.Parameters;
  for i := 0 to ap.Count-1 do
  begin
    if VarIsNull(ap[i].Value)
      then ps := '<Null>'
...
procedure TfSQLMon.ClearClick(Sender: TObject);
begin
