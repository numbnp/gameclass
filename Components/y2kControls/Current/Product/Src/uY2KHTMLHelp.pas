...
type
  ...
  protected
    procedure Change();
    procedure SetHelpFileName(const AstrHelpFileName: string);
  public
  ...
  published
    // properties
    property HelpFileName: string read FstrHelpFileName write SetHelpFileName;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
  end; // TY2KHTMLHelp
...
begin
  ...
  if Assigned(FlstClients) then begin
    while FlstClients.Count > 0 do
      UnRegisterChanges(TY2KHTMLHelpLink(FlstClients.Last));
//    Change();
//    FlstClients.Clear();
    FreeAndNil(FlstClients);
  end;
...
procedure TY2KHTMLHelp.Change();
var i: integer;
begin
  if FlstClients <> nil then
    for i := 0 to FlstClients.Count - 1 do
      TY2KHTMLHelpLink(FlstClients[i]).Change;
  if Assigned(FOnChange) then
    FOnChange(Self);
end; // TY2KHTMLHelp.Change

procedure TY2KHTMLHelp.SetHelpFileName(const AstrHelpFileName: string);
begin
  FstrHelpFileName := AstrHelpFileName;
  Change();
end;
