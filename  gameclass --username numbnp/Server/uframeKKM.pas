...
type
  ...
  TframeKKM = class(TFrame)
    ...
    rbtnOnlyResultCheck: TRadioButton;
    rbtnCashInForPrepay: TRadioButton;
    rbtnRefundForChange: TRadioButton;
    ...
    procedure rbtnOnlyResultCheckClick(Sender: TObject);
  private
...
begin
  ...
  rbtnOnlyResultCheck.Enabled := cbxKKM.Checked;
  rbtnCashInForPrepay.Enabled := cbxKKM.Checked;
  rbtnRefundForChange.Enabled := cbxKKM.Checked;
  ...
  if cbxKKM.Checked then begin
    ...
    case GRegistry.Modules.KKM.CheckMode of
      cmOnlyResultCheck: rbtnOnlyResultCheck.Checked := True;
      cmCashInForPrepay: rbtnCashInForPrepay.Checked := True;
      cmRefundForChange: rbtnRefundForChange.Checked := True;
    end;
...
procedure TframeKKM.rbtnOnlyResultCheckClick(Sender: TObject);
begin
  if GbframeKKMLock then exit;  GbframeKKMLock := True;
  if rbtnOnlyResultCheck.Checked then
    GRegistry.Modules.KKM.CheckMode := cmOnlyResultCheck
  else if rbtnCashInForPrepay.Checked then
    GRegistry.Modules.KKM.CheckMode := cmCashInForPrepay
  else if rbtnRefundForChange.Checked then
    GRegistry.Modules.KKM.CheckMode := cmRefundForChange;
  _AfterControlDataChange(True);
end;
