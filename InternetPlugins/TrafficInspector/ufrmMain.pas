...
uses
  ...
  uOptions, uY2KLinkLabel, uY2KHelpLabel, uY2KHTMLHelp;
...
begin
  ...
  if TI.Active then begin
    edtAPIState.Text := '  API: Интерфейс доступен!';
    edtAPIState.Font.Color := clGreen;
    btnAPITest.Enabled := True;
  end else begin
    edtAPIState.Text := '  API: Интерфейс недоступен!';
  ...
  if (Length(Options.Genaral.Password.Value) > 0) then begin
    ...
    //в принципе не нужно, т.к. TEdit сам убирает #00
    //coder._RemoveZeroBytes(edtPassword.Text);
    coder.Free;
  end else begin
    edtPassword.Text := '';
  end;
...
begin
  TI := TTICommonInterface.Create(Options.Genaral.DecriptedPassword);
  if TI.Logon(edtPassword.Text, strErrorMessage) then begin
    edtAPIState.Font.Color := clGreen;
    edtAPIState.Text := '  API: Подключение выполнено';
  end else begin
    edtAPIState.Font.Color := clMaroon;
